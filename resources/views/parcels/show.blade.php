            ...
            <div class="col-lg-4 col-md-6">
                <div class="info-card">
                    ...
                    <table class="table table-striped mt-4" style="width:100%;text-align:center">
                        ...
                        <tr><td>{{ $item->product->reference }} - {{ $item->product->name }} </td><td>{{$item->unit_price}}</td><td>{{$item->quantity}}</td><td>{{ number_format($item->unit_price * $item->quantity, 2) }} <sup>TND</sup></td></tr>
                        @endforeach
                    </table>
                </div>
            </div>
