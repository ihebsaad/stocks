    ...
    private function buildDataTable($orders, Request $request, $isArchive = false)
    {
        $dataTable = DataTables::of($orders)
            ->addColumn('client_name', function ($order) {
                ...
                if ($order->client) {
                    return '<div class="'.$class.'"><a href="' . route('clients.show', $order->client->id) . '" class="btn btn-sm btn-primary mr-1 mb-1" title="Voir"></a>'.$order->client->full_name . '</a><br><small>' . $order->client->phone . '</small></div>';
                }
